...
function personalizeCached(cachedData, userConcepts) {
  const conceptSource = (cachedData.allConcepts ?? []).map((c) => ({
    conceptId: c.conceptId,
    name: c.name,
    normalizedName: c.normalizedName ?? c.conceptId,
    category: c.category ?? "General",
    importanceScore: c.importanceScore ?? 50,
  }));

  const ranked = rankLearningConcepts(conceptSource, userConcepts);
  const gapMap = new Map((cachedData.knowledgeGaps ?? []).map((g) => [g.conceptId, g]));
  const knowledgeGaps = ranked.map((r) => gapMap.get(r.conceptId)).filter(Boolean);
