...
export async function runActionPlanner(articleId, options = {}) {
  ...
  if (signalStrength < 15 && (article.importanceScore ?? 0) < 55) {
    ...
    await saveActionPlannerResult(article.articleId, empty, "deterministic");
  ...
  if (!candidatesResult.success) {
    const fallback = formatActionPlannerResult({
      article,
      status: "NO_ACTION_REQUIRED",
      actions: [],
      events,
      rejected: [],
      signals,
      reason: "Action analysis could not complete right now. You can still read the story safely.",
      scores: { relevanceScore: 0, urgencyScore: signalStrength, confidenceScore: 0 },
    });
    await saveActionPlannerResult(article.articleId, fallback, "fallback");
    return { success: true, data: fallback, cached: false, model: "fallback" };
  }
  ...
  if (!candidatesResult.actionRequired || !candidatesResult.candidates?.length) {
    ...
    await saveActionPlannerResult(article.articleId, empty, candidatesResult.model);
  ...
  await saveActionPlannerResult(article.articleId, result, candidatesResult.model);

  return { success: true, data: result, cached: false, model: candidatesResult.model };
}

async function saveActionPlannerResult(articleId, result, model) {
  try {
    await cacheIntelligence(articleId, "actionPlanner", result, model);
  } catch (err) {
    console.error("[ActionPlanner] cache failed:", err);
  }

  try {
    await persistActionAnalysis(articleId, result, model);
  } catch (err) {
    console.error("[ActionPlanner] persist failed:", err);
  }
}
